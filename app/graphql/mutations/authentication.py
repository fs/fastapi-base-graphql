...
def user_sign_in(input: SignInInput, info: Info) -> TokenPairType:
    ...
    return TokenPairType(access_token=access_token, refresh_token=refresh_token, me=current_user)


def new_token_pair(input: Any, info: Info) -> TokenPairType:
    ...
    return TokenPairType(access_token=new_access_token, refresh_token=new_refresh_token, me=info.context['current_user'])
...
def user_sign_out(input: SignOutInput, info: Info) -> SignOutType:
    if not input.everywhere:
        ...
        crud_refresh_token.refresh_token.revoke_all_for_user(info.context['current_user'].id)
        return SignOutType(message='User sign out successfully')
